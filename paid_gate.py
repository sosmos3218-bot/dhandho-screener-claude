...
list_paid_subscribers/add_paid_subscriber/remove_paid_subscribers 는 결제 자동화(webhook/)를
붙이기 전, 관리자가 직권으로 유료 구독자를 관리하는 함수다. 관리자 페이지(app.py ?admin=1,
admin_page.py)와 로컬 CLI(scripts/manage_paid_subscribers.py)가 이 함수들을 공유해서
"로컬에서만 되는 기능"과 "웹에서 되는 기능"이 어긋나지 않게 한다.
"""
import hashlib
import hmac
import json
...
def list_paid_subscribers() -> list:
    """현재 Brevo 유료 리스트의 전체 컨택(raw dict, attributes 포함). 미설정 시 빈 리스트."""
    key = config.brevo_api_key()
    list_id = config.brevo_paid_list_id()
    if not key or not list_id:
        return []
...
def remove_paid_subscribers(emails: list) -> dict:
    """유료 리스트에서 이메일들의 멤버십을 제거. 미설정 시 빈 dict."""
    key = config.brevo_api_key()
    list_id = config.brevo_paid_list_id()
    if not key or not list_id:
        return {}
    return brevo.remove_contacts_from_list(list_id, [e.strip().lower() for e in emails], key)
